...
class SheepScraper:
    ...
    def get_with_retry(self, url: str, max_retries: int = MAX_RETRIES) -> Optional[BeautifulSoup]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning("Request error: %s - %s (attempt %d)", e, url, attempt+1)
    ...
    def scrape_avito(self) -> List[Dict[str, Any]]:
        ...
        for page in range(1, MAX_PAGES + 1):
            ...
            logger.info("Avito page %d: %s", page, url)
            ...
            if not cards:
                logger.warning("No cards found on Avito page %d", page)
                break
            ...
            logger.info("Avito page %d: %d listings", page, len([l for l in listings if l['source'] == 'avito.ma']))
    ...
    def parse_avito_card(self, card, base_url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("Error parsing Avito card: %s", e)
            return None
    ...
    def scrape_lebergier(self) -> List[Dict[str, Any]]:
        ...
        logger.info("Le Bergier: %d listings", len(listings))
        return listings
    ...
    def scrape_moutonika(self) -> List[Dict[str, Any]]:
        ...
        logger.info("Moutonika: %d listings", len(listings))
        return listings
    ...
    def scrape_all(self) -> List[Dict[str, Any]]:
        ...
        logger.info("Total unique listings: %d", len(unique_listings))
    ...
    def save_json(self, listings: List[Dict[str, Any]], filename: str = "sheep_dataset_real.json"):
        ...
        logger.info("Saved %d listings to %s", len(listings), filename)
